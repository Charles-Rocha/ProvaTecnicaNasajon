{ ************************************************************************************** }
{                                                                                        }
{ Tiny XML parser of OpenOffice (.OXT) dictionaries, for Delphi XE11+                    }
...
{                                                                                        }
{ Copyright 2010-2025 (C) Alexander Halser (http://www.ec-software.com)                  }
{                                                                                        }
{ ************************************************************************************** }
...
type
  ...
  private
    ...
    function Decode(const AValue: AnsiString): String;
  public
    constructor Create(AReader: TOxtParser);
    destructor Destroy; override;
    function ParentName: String;
    function NodeByName(const AName: String): TOxtNode;
    procedure NodesByName(const AName: String; var AList: TList);
    function Attrib(const AName: String): String;
...
function TOxtNode.ParentName: String;
begin
  Result := '';
...
function TOxtNode.NodeByName(const AName: String): TOxtNode;
var
  intItem: Integer;
begin
  Result := nil;
...
procedure TOxtNode.NodesByName(const AName: String; var AList: TList);
var
  intItem: Integer;
begin
  for intItem := 0 to FItems.Count - 1 do
...
function TOxtNode.Decode(const AValue: AnsiString): String;
...
const
  escapes: array [0 .. 4, 0 .. 1] of String = (('&', '&amp;'), ('<', '&lt;'), ('>', '&gt;'), ('''', '&apos;'), ('"', '&quot;'));
var
  l: Integer;
begin
  Result := '';
  ...
  begin
    l := MultiByteToWideChar(FReader.FCodepage, 0, PAnsiChar(AValue), -1, Nil, 0);
    if l > 1 then
    begin
      SetLength(Result, l - 1);
      MultiByteToWideChar(FReader.FCodepage, 0, PAnsiChar(AValue), -1, PWideChar(Result), l - 1);
    end;
  end;

  for l := 0 to 4 do
    Result := StringReplace(Result, escapes[l][1], escapes[l][0], [rfReplaceAll, rfIgnoreCase]);
end;

function TOxtNode.GetName: String;
begin
  Result := Decode(FName);
end;

function TOxtNode.GetValue: String;
begin
  Result := Decode(FValue);
end;

function TOxtNode.GetAttr: String;
begin
  Result := Decode(FAttr);
end;

function TOxtNode.Attrib(const AName: String): String;
var
  tmpStr: TStringlist;
begin
  tmpStr := TStringlist.Create;
  try
    tmpStr.Text := Decode(FAttr);
    Result := tmpStr.Values[AName];
  finally
    FreeAndNil(tmpStr);
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      newCodepage := 20866
    else if (Copy(tmpEncoding, 1, 7) = 'windows') or (Copy(tmpEncoding, 1, 9) = 'microsoft') then
      newCodepage := StrToIntDef(Copy(tmpEncoding, 4, Length(tmpEncoding) - 3), FCodepage);  { implicit conversion }

    if newCodepage <> FCodepage then
    begin
      FCodepage := newCodepage;
      FUTF8encoded := false;
    end;
  end;
...
begin
  ...
  begin
    ...
    str.Text := Node.Decode(Node.FAttr);
