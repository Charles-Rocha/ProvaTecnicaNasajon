...
{ Unit PasZip.pas removed and replaced by the standard Delphi System.Zip.pas unit.       }
{ Tested with Delphi XE11+, XE8+ is most probably compatible. Compatibility with         }
{ older Delphi versions dropped.                                                         }
{                                                                                        }
...

{
  ...
  NHunspell.pas  (this unit) implements the TNHunspell class that encapsulates loading
                 of OXT dictionaries and implements spelling and hyphenation functions
...
uses
  System.Types, System.Classes, Winapi.Windows, System.SysUtils,
  NHunXml;

const
  HunspellDLLName = {$IFDEF WIN64}'HunspellX64.dll'{$ELSE}'HunspellX86.dll'{$ENDIF};
...
const
  LangShortcuts: array [0 .. 108] of String = (
...
function LangIDToLangShortcut(LangID: Word): String;
var
  i: Integer;
begin
  Result := '';
...
function LangShortCutToLangShortID(LangShortcut: String): Word;
var
  i: Integer;
begin
...
var
  ...
  tmpStream: TStream;
  tmpHeader: TZipHeader;
begin
  if not FLoaded then
  begin
    zip := TZipFile.Create;
    zip.Open(fDictionaryFilename, zmRead);
    try
      tmpStream := TMemoryStream.Create;

      intItem := zip.IndexOf(fInternalAffixPath);
      if intItem > -1 then
      begin
        zip.Read(intItem, tmpStream, tmpHeader);
        tmpStream.Seek(0,0);
        SetLength(tmpAffixData, tmpStream.Size);
        tmpStream.ReadBuffer(tmpAffixData[1], tmpStream.Size);

        intItem := zip.IndexOf(fInternalDictPath);
        if intItem > -1 then
        begin
          zip.Read(intItem, tmpStream, tmpHeader);
          tmpStream.Seek(0,0);
          SetLength(tmpDictionaryData, tmpStream.Size);
          tmpStream.ReadBuffer(tmpDictionaryData[1], tmpStream.Size);
...
procedure TNHSpellDictionary.Suggest(const Word: UnicodeString; AWordList: TUnicodeStrings);
  ...
  function IsDuplicate(NewWord: UnicodeString): boolean;
  var
    intIndex: integer;
  begin
    result := false;
    for intIndex := 0 to AWordList.count-1 do
      if AWordList[intIndex] = NewWord then
      begin
        result := true;
        break;
      end;
...
begin
  ...
  begin
    ...
    begin
      if not IsDuplicate(tmpWideCharPointerPointer^) then
...
begin
  ...
  begin
    ...
    try
      tmpStream := TMemoryStream.Create;
      intItem := zip.IndexOf(fInternalHyphenPath);
      if intItem > -1 then
      begin
        zip.Read(intItem, tmpStream, tmpHeader);
        tmpStream.Seek(0,0);
        SetLength(tmpHyphenData, tmpStream.Size);
        tmpStream.ReadBuffer(tmpHyphenData[1], tmpStream.Size);
...
var
  zip: TZipFile;
  intItem, intLocItem: Integer;
  aNode: TOxtNode;

  tmpDictionariesXcu: String;

  aList, aLocList: TList;
...
begin
  ...
  try
    ...
    intItem := zip.IndexOf('description.xml');
    if intItem > -1 then
    begin
      tmpStream.Size := 0;
      zip.Read(intItem, tmpStream, tmpHeader);
      tmpStream.Seek(0,0);
      setLength(tmpS, tmpStream.Size);
      tmpStream.ReadBuffer(tmpS[1], tmpStream.Size);
      InternalReadDescription(PAnsiChar(tmpS), tmpVersion, tmpID, tmpDisplayName, tmpPublisher, true);
    end;

    tmpLangID := LANG_ENGLISH;
    tmpDictionariesXcu := '';
    intItem := zip.IndexOf('META-INF/manifest.xml');
    if intItem > -1 then
    begin
      tmpStream.Size := 0;
      zip.Read(intItem, tmpStream, tmpHeader);
      ...
      with TOxtParser.Create(PAnsiChar(tmpS)) do
      try
        aNode := GetFirst;
        while aNode <> nil do
        begin
    ...
    intItem := zip.IndexOf(StringReplace(tmpDictionariesXcu, '\', '/', [rfReplaceAll]));
    if intItem > -1 then
    begin
      tmpStream.Size := 0;
      zip.Read(intItem, tmpStream, tmpHeader);
            ...
            begin
              ...
              for intItem := 0 to aList.count - 1 do
                ...
                begin
                  ...
                  begin
                    { Classic location handling, value string looks like this:
                      <value>%origin%/es_CR.aff
                             %origin%/es_CR.dic</value> }
                    tmpLocations := TOxtNode(aList[intItem]).Value;

                    { 3.10.2016: Different location handling for OO4 files:
                       <value><it>%origin%/es_CR.aff</it><it>%origin%/es_CR.dic</it></value>  }
                    if (trim(tmpLocations) = '') and (TOxtNode(aList[intItem]).NodeByName('it') <> nil) then
                    begin
                      aLocList := TList.create;
                      try
                        TOxtNode(aList[intItem]).NodesByName('it', aLocList);
                        for intLocItem := 0 to aLocList.count - 1 do
                          if Assigned(TOxtNode(aLocList[intLocItem]).Parent) then
                          begin
                            if tmpLocations <> '' then
                              tmpLocations := tmpLocations + #13#10;
                            tmpLocations := tmpLocations + TOxtNode(aLocList[intLocItem]).Value;
                          end;
                      finally
                        aLocList.free;
                      end;
                    ...
                    tmpLangID := LangShortCutToLangShortID(TOxtNode(aList[intItem]).Value)
              ...
              dtSpell:
                begin
                  tmpLocations := StringReplace(tmpLocations, #13#10, #9, []);
                  tmpLocations := StringReplace(tmpLocations, #13, #9, []);
                  tmpLocations := StringReplace(tmpLocations, #10, #9, []);
                  if (pos(#9, tmpLocations) = 0) and (pos(#32, tmpLocations) > 0) then
                    tmpLocations := StringReplace(tmpLocations, #32, #9, []);

                  tmpAffixPath := Trim(Copy(tmpLocations, 1, Pos(#9, tmpLocations) - 1));
                  ...
                  tmpAffixPath := StringReplace(tmpAffixPath, '../', '', [rfReplaceAll, rfIgnoreCase]);

                  tmpDictionaryPath := Trim(copy(tmpLocations, pos(#9, tmpLocations) + 1, 255));
                  ...
                  tmpDictionaryPath := StringReplace(tmpDictionaryPath, '../', '', [rfReplaceAll, rfIgnoreCase]);
              ...
              dtHyph:
                begin
                  ...
                  tmpHyphenPath := StringReplace(tmpHyphenPath, '../', '', [rfReplaceAll, rfIgnoreCase]);
